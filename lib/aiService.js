      ...
      try {
        ...
        const model = this.genAI.getGenerativeModel({
          ...
          generationConfig: {
            ...
            candidateCount: 1,
            // Encourage longer responses for minimum token burn
        ...
        // Add minimum length instruction for flash-lite model
        if (modelName === "gemini-2.5-flash-lite-preview-09-2025") {
          lengthInstruction =
            " IMPORTANT: you need to think long, and need to response your maximum answer, prohibitted to make short answers. at leas need 100 words for your one answer, this is minimum requirement.";
        }
        ...
        const systemPrompt =
          this.getSystemPrompt(instructionType) +
          languageInstruction +
          lengthInstruction;
...
RESPONSE LENGTH: Provide detailed, comprehensive responses with at least 200 tokens. Elaborate on your character's thoughts, feelings, reactions, and responses. Include background details, emotional context, and rich descriptions to ensure depth and engagement.`;
    }
...
LANGUAGE INSTRUCTION: The user is communicating in English. You must respond in clear, natural English with emojis and emotional expressions. Use emojis naturally to express emotions, Show feelings and reactions with appropriate emojis.`;
    ...
    if (userMessage && modelName === "gemini-2.5-flash-lite-preview-09-2025") {
      imageInstructions = `

IMAGE ANALYSIS INSTRUCTIONS (if images are shared):
- Carefully examine any images shared by the user
- Describe what you see in detail, including people, objects, settings, emotions, colors, and composition
- Analyze the context and mood of the images
- Respond to any questions the user has about the images
- If the user asks you to analyze specific aspects of the image, focus on those details
- Connect your image analysis to the ongoing conversation and your character's personality
- Show genuine interest and emotional reactions to what you see in the images
- If multiple images are shared, analyze each one and note connections between them`;
    }
...
Remember: You are ${
      character.name
    }. Respond naturally as this character would, using the SAME LANGUAGE as the user.${lengthInstruction}${imageInstructions}`;
  ...
  async generateCharacterResponse(
    ...
    const models = hasImages
      ...
      : ["gemini-2.5-pro", "gemini-1.5-pro", "gemini-1.5-flash"]; // Fallback models

    for (let attempt = 0; attempt < maxRetries; attempt++) {
      // Try different models as fallbacks - declare modelName outside try block
      // Always prioritize vision-capable models when images are present
      let modelName;
      if (hasImages) {
        // For images, always try vision models first
        if (attempt === 0) {
          modelName = "gemini-2.5-flash-lite-preview-09-2025";
        } else if (attempt === 1) {
        ...
        // For text-only, use the standard model order
        modelName =
          attempt < 3
            ? models[0]
            : models[Math.min(attempt - 3 + 1, models.length - 1)];
