...
class DatabaseService {
  ...
  async addMessage(
    ...
    const query = `
      INSERT INTO messages (session_id, role, content, token_count, metadata, images, has_images)
    ...
    const values = [
      ...
      hasImages ? JSON.stringify(images) : null,
      hasImages,
    ];
  ...
  async getMessages(sessionId, limit = 50) {
    ...
    // Parse images JSON data for each message
    return messages.map((message) => ({
      ...message,
      images: this.parseJsonSafely(message.images),
      metadata: this.parseJsonSafely(message.metadata),
    }));
  }
  ...
  // Paid Plan Interest operations
  async submitPaidPlanInterest(submissionData) {
    const {
      userId,
      userEmail,
      userName,
      interestedInPaidPlan,
      budgetRange,
    ...
    const query = `
      INSERT INTO paid_plan_interest (
        user_id, user_email, user_name, interested_in_paid_plan,
        budget_range, current_usage_frequency, user_agent, ip_address
  ...
  async getPaidPlanSubmissions(userId = null, limit = 50) {
    let query = `
      SELECT * FROM paid_plan_interest
      WHERE 1=1
    `;
    const values = [];
    let paramCount = 1;
  ...
  async getPaidPlanStats() {
    const query = `
      SELECT
        COUNT(*) as total_submissions,
        COUNT(CASE WHEN interested_in_paid_plan = true THEN 1 END) as interested_count,
        COUNT(CASE WHEN interested_in_paid_plan = false THEN 1 END) as not_interested_count,
        COUNT(DISTINCT user_id) as unique_users,
        json_agg(DISTINCT preferred_plan_type) FILTER (WHERE preferred_plan_type IS NOT NULL) as plan_types,
        json_agg(DISTINCT budget_range) FILTER (WHERE budget_range IS NOT NULL) as budget_ranges
